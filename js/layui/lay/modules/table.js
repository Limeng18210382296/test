...
;
layui.define(["laytpl","jquery", "laypage", "excel","layer", "form", "util"],
function(e) {
	"use strict";
	var $ = layui.jquery;
	var t = layui.$,
	...
	u = function(e, a, l, n) {
		...
		a = e.format ? function(){
			var reg = /\B(?=(\d{3})+$)/g;
			var num =(a.toString().indexOf('.') != -1 ? a.toString().split('.') : a);
			if(Array.isArray(num)){
				return String(num[0]).replace(reg, ',') + '.' + num[1];
			}else{
		...
		if (i.startTime = (new Date).getTime(), a.url) {
			...
			t.ajax({
				...
				success: function(t) {
					...
					"function" == typeof a.sortDone && a.sortDone()
				},
				error: function(e, t) {
			...
			o(),
			i.setColsWidth(),
			"function" == typeof a.done && a.done(c, e, c[n.countName]),"function" == typeof a.sortDone && a.sortDone()
		}
	},
	...
	F.prototype.renderData = function(e, n, o, r) {
		...
		b = function() {
			...
			50), c.haveInit = !0,
			(l.close && l.close(c.tipsIndex), s.HAS_SET_COLS_PATCH || c.setColsPatch(), void(s.HAS_SET_COLS_PATCH = !0)))
		};
	...
	F.prototype.sort = function(e, i, a, l) {
		...
		(u.noSortRow && r.map(function(v,index){if(v['LAY_TABLE_INDEX'] == (u.noSortRow-1)){ r.splice(index,1); r.unshift( v);}})),
		s[u.response.dataName] = r || f,
		c.renderData(s, c.page, c.count, !0),
		l && layui.event.call(e, y, "sort(" + h + ")", {
			field: n,
			type: i
		}),(u.sortDone && u.sortDone())
	},
	...
	F.prototype.events = function() {
		...
		function(e) {
			...
			switch (layui.stope(e), j.trigger("table.tool.panel.remove"),l.close && l.close(i.tipsIndex), c) {
			case "LAYTABLE_COLS":
				s({
					list:
					function() {
						...
						return i.eachCols(function(t, i) {
							if(i.field && "normal" == i.type){
								if(i.hide){ alloff = true; }
						...
						e.unshift('<li><input type="checkbox" name="colsall" data-key="0-all" data-parentkey ' + (alloff ? "": "checked") + ' lay-skin="primary" title="全部" lay-filter="LAY_TABLE_TOOL_COLS"></li>'),
						e.join("")
					} (),
					done: function() {
						...
						function(e) {
							...
							function(e, t) {
								if(o === "0-all"){
									var loadIndex = null, isok = false
									layui.layer.confirm('数据量过大的话,全选会加载稍慢,是否要全选?', {icon: 3, title:'提示',
										end:function(){
											if(isok){
												layui.each(t, function(t, l) {
													l.hide = !n;
												});
												layui.table.render(a);
												 layer.close(loadIndex);
											}
										}}, function(index){
										  //do something
										 loadIndex = layer.load(1,{ zIndex : 999999999  }); //换了种风格
										 isok = true;
										 layer.close(index);
									});
									return;
									...
									offall = t.filter(function(v){ return v.hide === true });
									if(!offall.length){
										i.elem.find('input[name="colsall"]').prop("checked",true);
										layui.form.render('checkbox');
									}else{
										i.elem.find('input[name="colsall"]').prop("checked",false);
										layui.form.render('checkbox');
									}
								}
							})
	...
	d.exportFile = function(e, t, i) {
		...
		if(i == 'xlsx'){
			var theadObj = (function(){
				...
				dataList.unshift(i);
				excel.downloadExl(dataList, (l.title || "table_" + (l.index || "")),'xlsx')
				//i.join(",") + "\r\n" + l.join("\r\n") + "\r\n" + n.join(",")
			})()
			/*
			 * 	var tableObj = $('[lay-id="'+e+'"]').find('.layui-table-box > .layui-table-main table').clone();
			var theadObj = $('[lay-id="'+e+'"]').find('.layui-table-box > .layui-table-header table').clone();
			
			tableObj.find('td').each(function(){ $(this).text($(this).text()); });
			theadObj.find('th').each(function(){ $(this).text($(this).text()); })
			theadObj.append(tableObj);

	    	var excelFile = "<html xmlns:o='urn:schemas-microsoft-com:office:office' xmlns:x='urn:schemas-microsoft-com:office:excel' xmlns='http://www.w3.org/TR/REC-html40'>";
	    	excelFile += "<head><!--[if gte mso 9]><xml><x:ExcelWorkbook><x:ExcelWorksheets><x:ExcelWorksheet><x:Name>{worksheet}</x:Name><x:WorksheetOptions><x:DisplayGridlines/></x:WorksheetOptions></x:ExcelWorksheet></x:ExcelWorksheets></x:ExcelWorkbook></xml><![endif]--></head>";
	        excelFile += "<body><table width='10%'  border='1'>";
	        excelFile += theadObj[0].innerHTML;
	        excelFile += "</table></body>";
	        excelFile += "</html>";
	        var link = "data:application/vnd.ms-excel;base64," + window.btoa(unescape(encodeURIComponent(excelFile)));
		 
			s.download = (l.title || "table_" + (l.index || "")) + '.xlsx';
			s.href = link;
			s.click();
			s.remove();
			 * */
